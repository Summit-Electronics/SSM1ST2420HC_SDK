...
void TMC5160_Basic_Init(CurrentConfig *Current)
{
	...
	*/
	...
	TMC5160_SPIWrite(0x39, 0x00000000, 1, &SPICheckValue); // writing value 0x00000000 = 0 = 0.0 to address 28 = 0x39(X_ENC)
	...
	TMC5160_SPIWrite(0x70, 0xC40C001E, 1, &SPICheckValue); // writing value 0xC40C001E = 0 = 0.0 to address 44 = 0x70(PWMCONF)
	  ...
	  if(ENC_ENB == 1)
	  {
		  ...
		  ENC_Start_position(); // set encoder current position to 0
	  }
...
void TMC5160_Rotate_To(uint32_t Position, RampConfig *Ramp)
{
	...
	//while(TMC_Pos != Target_Pos)
	{
		...
		{
			ENC_Pos = (ENC_Get_Position()) * ENC_Factor;
		}
...
{
	uint32_t Enc_Position = TMC5160_SPIWrite(0x39, 0x00000000, 0, &SPICheckValue); // read encoder position
...
void ENC_Start_position(void)
{
	...
	TMC5160_SPIWrite(0x39, 0x00000000, 0, &SPICheckValue); // read encoder position
}
